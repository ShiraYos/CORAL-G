...
class DebrisPredictionNode(Node):
    def __init__(self):
        ...
        self._recent_events = []
        self._belief_cells = {}
        self._recent_collection_locations: dict = {}  # (rx, ry) -> sim_time_sec, dedup guard
    ...
    def _goal_reached_cb(self, msg: String):
        ...
        if x is not None and y is not None:
            self._apply_collection_to_belief(float(x), float(y), 1)
            self.get_logger().info(
                f'Goal reached — updated belief density near ({x:.2f}, {y:.2f})'
            )
